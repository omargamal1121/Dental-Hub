...
namespace DentalHub.API.Controllers.v2
{
    ...
    public class DiagnosesController : BaseController
    {
        ...
        public async Task<ActionResult<ApiResponse<PagedResult<DiagnosisDto>>>> GetByNationalId(string nationalId, [FromQuery] int page = 1, [FromQuery] int pageSize = 10)
        {
            var result = await _mediator.Send(new GetDiagnosesByNationalIdQuery(nationalId, page, pageSize));
            return HandleResult(result);
        }
