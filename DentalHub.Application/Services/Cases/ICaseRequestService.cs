...
namespace DentalHub.Application.Services.Cases
{
    public interface ICaseRequestService
    {
        ...
        // Get Requests
        Task<Result<PagedResult<CaseRequestDto>>> GetRequestsByDoctorIdAsync(string doctorPublicId, int page = 1, int pageSize = 10);
        Task<Result<PagedResult<CaseRequestDto>>> GetRequestsByStudentIdAsync(string studentPublicId, int page = 1, int pageSize = 10);
		...
		Task<Result<IEnumerable<CaseRequestDto>>> GetRequestsByCaseIdAsync(string casePublicId, RequestStatus? status = null);
