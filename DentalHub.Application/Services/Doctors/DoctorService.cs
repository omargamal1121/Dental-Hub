...
namespace DentalHub.Application.Services.Doctors
{
    public class DoctorService : IDoctorService
    {
        ...
        // للدكتور نفسه - بيدور بـ UserId (اللي جاي من الـ JWT Token)
        public async Task<Result<DoctorDto>> GetDoctorByIdAsync(string userId)
        {
            try
            {
                // تحويل الـ string لـ Guid عشان EF Core يقدر يترجمه لـ SQL صح
                if (!Guid.TryParse(userId, out var userGuid))
                    return Result<DoctorDto>.Failure("Invalid user ID");

                var spec = new BaseSpecificationWithProjection<Doctor, DoctorDto>(
                    d => d.UserId == userGuid,  // مقارنة Guid بـ Guid ✅
                    d => new DoctorDto
                    {
                        PublicId = d.PublicId,
                        FullName = d.User.FullName,
